...
def parsing(dataset_name, output_dir = '/Dataset'):
    ...
    if dataset_name == 'BGL':
        ...
        parser = DrainThunderbird.LogParser(log_format, indir=input_dir, outdir=output_dir,  depth=depth, st=st, rex=regex)
        parser.parse(log_file)
        ...
        try:
            shutil.move('anomaly_label.csv', 'Dataset/anomaly_label.csv')
            os.remove(log_file)
        except OSError:
            pass
